...
export function AccountForm({ initialValues, onSubmit, onCancel }: AccountFormProps): JSX.Element {
  ...
  const [errors, setErrors] = useState<Partial<Pick<Account, 'name' | 'type'>>>({});
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!result.success) {
      const fieldErrors: Partial<Pick<Account, 'name' | 'type'>> = {};
      for (const issue of result.error.errors) {
        const field = issue.path[0] as keyof Pick<Account, 'name' | 'type'>;

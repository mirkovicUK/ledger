...
export function LedgerProvider({ children }: { children: ReactNode }) {
  const [state, dispatch] = useReducer<AppState, LedgerAction>(ledgerReducer, INITIAL_STATE);
  ...
  useEffect(() => {
    loadState()
      ...
      .catch(() => {
        setStorageError(
          'Failed to load your saved data. It may be corrupted or unavailable.'
        );
      });
  }, []);
  ...
  // Memoized selectors
  const totalBal = useMemo(() => totalBalance(state.transactions), [
    state.transactions,
  ]);
  const accountBalances = useMemo(() => {
    return Object.fromEntries(
      state.accounts.map((a) => [
        a.id,
        accountBalance(state.transactions, a.id),
      ])
    );

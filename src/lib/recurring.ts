...
export function expandRule(
  ...
  if (rule.lastExpandedDate == null) {
    ...
    currentDate = advanceByFrequency(
      parseDate(rule.lastExpandedDate),
      rule.frequency
    );
  }
...
export function expandAllRules(
  rules: RecurringRule[],
  existingTransactions: Transaction[],
  referenceDate: string,
  idGenerator: () => string
): ExpandAllRulesResult {
  const allNewTransactions: Transaction[] = [];
  const updatedRules: RecurringRule[] = [];

  for (const rule of rules) {
    const { transactions: newTxs, updatedRule } = expandRule(
      rule,
      referenceDate,
      idGenerator
    );

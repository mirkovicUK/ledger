...
function makeTx(categoryId, date, amount) {
...
describe('computeBudgetSpending', () => {
  test('returns 0 when transaction array is empty (Req 4.5)', () => {
    expect(computeBudgetSpending(foodBudget, [], REF)).toBe(0);
  });

  test('returns 0 when no transactions match the category', () => {
    const txs = [
  ...
  test('sums transactions that match category and fall within the period', () => {
    const txs = [
  ...
  test('excludes transactions outside the period even if category matches', () => {
    const txs = [
  ...
  test('excludes transactions with wrong category that are in the period', () => {
    const txs = [
  ...
  test('excludes transactions where categoryId is null or undefined', () => {
    const txs = [
  ...
  test('includes boundary dates (first and last day of period)', () => {
    const txs = [
  ...
  test('returns 0 when category has no matching transactions (Req 4.5)', () => {
...
describe('isOverspent', () => {
  test('returns false when spent equals limit (Req 4.3)', () => {
    expect(isOverspent(300, 300)).toBe(false);
  });

  test('returns false when spent is below limit', () => {
    expect(isOverspent(100, 300)).toBe(false);
    expect(isOverspent(0, 300)).toBe(false);
  });

  test('returns true when spent exceeds limit (Req 4.3)', () => {
    expect(isOverspent(301, 300)).toBe(true);
    expect(isOverspent(1000, 300)).toBe(true);
  });

  test('returns false when both spent and limit are zero', () => {
    expect(isOverspent(0, 0)).toBe(false);
  });

  test('returns true for fractional overspend', () => {
    expect(isOverspent(100.01, 100)).toBe(true);
  });

  test('returns false for fractional underspend', () => {
    expect(isOverspent(99.99, 100)).toBe(false);
  });
});

describe('progressRatio', () => {
  test('returns 0 when limit is 0 to avoid division by zero', () => {
  ...
  test('returns 0.5 when half the budget is spent (Req 4.2)', () => {
  ...
  test('returns > 1 when overspent (Req 4.3)', () => {

...
import type { FilterCriteria } from '../src/lib/filter.js';

// Sample transaction fixture
const transactions = [
  { id: '1', accountId: 'acct-a', categoryId: 'cat-food' as string | null | undefined, date: '2024-01-10', amount: 50 },
  { id: '2', accountId: 'acct-a', categoryId: 'cat-rent' as string | null | undefined, date: '2024-01-15', amount: 1200 },
  { id: '3', accountId: 'acct-b', categoryId: 'cat-food' as string | null | undefined, date: '2024-02-05', amount: 30 },
  { id: '4', accountId: 'acct-b', categoryId: null as string | null | undefined,        date: '2024-02-20', amount: -100 },
  { id: '5', accountId: 'acct-a', categoryId: undefined as string | null | undefined,   date: '2024-03-01', amount: 200 },
];

describe('applyFilters — null / empty criteria', () => {
  test('returns all transactions when criteria is null', () => {
    expect(applyFilters(transactions, {} as FilterCriteria)).toEqual(transactions);
  });

  test('returns all transactions when all criteria fields are undefined', () => {
    expect(applyFilters(transactions, {})).toEqual(transactions);
  });

  test('returns all transactions when all criteria fields are explicitly null', () => {
...
describe('applyFilters — amountRange filter', () => {
  test('filters transactions within an inclusive amount range', () => {
...
describe('applyFilters — combined AND logic', () => {
  ...
  test('categoryId + amountRange narrows result', () => {
    const result = applyFilters(transactions, {
      categoryId: 'cat-food',
  ...
  test('all four criteria combined', () => {
    const result = applyFilters(transactions, {
      accountId: 'acct-a',
      categoryId: 'cat-food',
      dateRange: { from: '2024-01-01', to: '2024-01-31' },
  ...
  test('conflicting criteria produce an empty result', () => {
...
describe('applyFilters — empty transaction list', () => {
  test('returns empty array when given an empty array regardless of criteria', () => {

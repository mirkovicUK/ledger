...
describe('Property 9: Sort results are correctly ordered', () => {
  const FIELDS: SortConfig['field'][] = ['date', 'amount', 'description'];
  const DIRECTIONS: SortConfig['direction'][] = ['asc', 'desc'];
  ...
  function compareValues(a: TestTransaction, b: TestTransaction, field: SortConfig['field']): number {
    if (field === 'amount') {
